                        ...
                        <Grid container direction={'row'} sx={{ p: 0 }} justifyContent='start'>
                            ...
                            <Grid direction={'column'}>
                                ...
                                <Grid sx={{ p: 0, pt: 1, borderColor: 'primary.main' }} >
                                    <Button size="small" disabled={!props.user.attachments.pdf.length} sx={{ mr: 1 }} variant='contained' onClick={() => onAttachmentClick(props.user.attachments.pdf[0])}>PDF({props.user.attachments.pdf.length})</Button>
                                    <Button size="small" disabled={!props.user.attachments.docx.length} sx={{ mr: 1 }} variant='contained' > Docx({props.user.attachments.docx.length})</Button>
                                    <Button size="small" disabled={!props.user.attachments.excel.length} sx={{}} variant='contained'> Excel({props.user.attachments.excel.length})</Button>
                                </Grid>
                            </Grid>
                        </Grid>
                    </AccordionSummary>
